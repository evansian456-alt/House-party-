...
test.describe('New UX Flow - Landing, Tier Selection, and Account Creation', () => {
  ...
  test('Flow C - Party Pass: Landing → Party Pass Tier → Account Creation', async ({ page }) => {
    ...
    await expect(page.locator('text=£3.99 · Up to 4 phones · 2 hours')).toBeVisible(); // GBP - Party Pass pricing
  ...
  test('Landing Page Content Validation', async ({ page }) => {
    ...
    await expect(page.locator('text=£3.99').first()).toBeVisible(); // GBP - Party Pass pricing
    ...
    // Pro tier
    await expect(page.locator('text=£9.99 / month')).toBeVisible(); // GBP - Pro Monthly pricing

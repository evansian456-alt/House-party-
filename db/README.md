...
Use the automated setup script to create all required databases:

```bash
cd db
chmod +x setup-databases.sh  # Make script executable (if needed)
./setup-databases.sh
```

This script will:
- Check if PostgreSQL is running
- Create `phoneparty` (primary production database)
- Create `syncspeaker` (fallback database)
- Create `phoneparty_test` (test database)
- Apply schema to all databases
...
1. Create a database:
   ```bash
   sudo -u postgres createdb phoneparty
   ```

2. Apply the schema:
   ```bash
   sudo -u postgres psql -d phoneparty -f db/schema.sql
   ```

3. Apply migrations:
   ```bash
   sudo -u postgres psql -d phoneparty -f db/migrations/001_add_performance_indexes.sql
   ```
